...
import miyucomics.sokana.engine.environments.SpellEnvironment

// Temporary construct, does not ever get serialized
// Steps through a spell image to tick it forwards
class SpellEngine(var image: SpellImage, val env: SpellEnvironment) {
	fun executeAtoms(spell: List<Atom>) {
		...
		try {
			while (continuation is SpellContinuation.NotDone) {
				...
				val result = frame.step(this, env.world, continuation.next)
